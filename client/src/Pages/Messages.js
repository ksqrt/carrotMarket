...
function Messages({ match }) {
  // match = Router 제공 객체, url을 매개변수로 사용. ex) 경로 : /messages/123  => match.params.id = "123" // app.js 참고 : <Route path="/messages" exact component={Messages} />;
  ...
  const [selected, setSelected] = useState({
    // 선택된 채팅방의 상세 정보(참가user, conversation(나눈 대화 내역)) 저장
    chats: {
      _id: 0,
  ...
  useEffect(() => {
    const firstUrl =
      "https://veiled-jay-0c2.notion.site/image/https%3A%2F%2Fs3-us-west-2.amazonaws.com%2Fsecure.notion-static.com%2F9e0b0bbd-b7f7-4a2d-9ed8-dfe08f72c35f%2F1e917e59f980468a78f2bff7dcc25ac2f604e7b0e6900f9ac53a43965300eb9a.png?id=653f7765-1ec3-485c-8d54-2af4e2b0e6aa&table=block&spaceId=5989bf22-29e0-4423-b8aa-9d2d5f3b5c6b&width=420&userId=&cache=v2";
    const secondUrl =
      "https://veiled-jay-0c2.notion.site/image/https%3A%2F%2Fs3-us-west-2.amazonaws.com%2Fsecure.notion-static.com%2F1d1e1eb4-d168-41ad-900e-ca97dd8e3663%2Fi16595761484.jpg?id=8b345997-b188-420a-9b2c-1df970806512&table=block&spaceId=5989bf22-29e0-4423-b8aa-9d2d5f3b5c6b&width=730&userId=&cache=v2";
  ...
  useEffect(() => {
    ...
    getUserConversations(socket, userData._id) // 현재 사용자와 관련된 모든 채팅방 목록을 가져옴
      .then((res) => {
        ...
        if (isSelected) {
          // 채팅방이 선택되었다면 현재 선택된 채팅방의 정보를 selected 상태 변수에 저장
                    ...
                    >
                      {x.isBuyer ? (
                        <>
                          ...
                          {x.chats.product?.image && (
                            <img
                              src={x.chats.product?.image}
                              alt="product"
                              style={{
                                float: "right",
                                width: "35px",
              ...
              <div className="chat-selected-header col-lg-12">
                ...
                <div className="dropdown">
                  ...
                  <div className="dropdown-content">
                    <button
                      className="dropdown-content-out"
              ...
              {alertShow && (
                ...
                >
                  <div className="flex-container">
                    <img
                      src={selected.chats.product?.image}
                      alt="product"
                      className="img-style"
                    />
                    <div className="text-container">
                      <div>
                        <span className="text-bold">
                          {selected.chats.product?.soldout
                            ? "거래완료"
                            : "거래중"}
                        </span>{" "}
                        &nbsp;&nbsp;
                  ...
                  <button> 후기 보내기 버튼 </button>{" "}
                  {/* 약속 잡기 성공 후 sold out 시 */}
                  <button> 약속 잡기 버튼 </button>{" "}
                  {/* (다른 사람과 약속 잡기가 되있지 않을 때) */}
                </Alert>
              )}
              <div
                ref={chatContainerRef}
                id="chat-selected-body"
                className="chat-selected-body col-lg-12"
                style={{ backgroundImage: `url(${bgUrl})` }}
              >
                {selected.chats.conversation
                  .slice(
                    Math.max(
                      selected.chats.conversation.length - showMessagesCount,
                      0
                    )
                  )
                  .map((x, index) => {
                    if (x) {
                      const messageDate = new Date(x.sentAt).toLocaleDateString(
                        "ko-KR",
                          ...
                          {messageDate !== currentDate &&
                            (currentDate = messageDate) && (
                              <p className="dateHeader">
                                <div className="hr-sect">{currentDate}</div>
                              </p>
                            )}
                          <div
                            className={
                              selected.myId === x.senderId ? "me" : "not-me"
                            }
                          >
                            <span className="timestamp">
                              {x.sentAt
                                ? new Date(x.sentAt).toLocaleTimeString(
                                    "ko-KR",
                                    {
                                      hour: "numeric",
                                      minute: "numeric",
                    ...
                    >
                      <InputGroup.Append>
                        <nav className={styles.menu}>
                          ...
                          <button
                            type="button"
                            className={`${styles["menu-item"]} ${styles.blue}`}
                            onClick={() =>
                          ...
                          <button
                            className={`${styles["menu-item"]} ${styles.green}`}
                          >
                            {" "}
                            <AiOutlineSchedule
                              className="upload-icon"
                          ...
                          <button
                            className={`${styles["menu-item"]} ${styles.red}`}
                          >
                            {" "}
                            <div
                          ...
                          <button
                            className={`${styles["menu-item"]} ${styles.purple}`}
                          >
                            {" "}
                          </button>
                          <button
                            className={`${styles["menu-item"]} ${styles.orange}`}
                          >
                            {" "}
                          </button>
                          <button
                            className={`${styles["menu-item"]} ${styles.lightblue}`}
                            onClick={onOpen}
                          >
                            {" "}
                            <FaMapMarkedAlt
